...
pipedat <- rawdat %>% 
  ...
  group_by(year) %>%
  mutate(annualrank = rank(-emissions)) %>% #rank emissions in descending order
  filter(year == 2015,
         annualrank <= 10)
```
...
```{r ggplot, eval=FALSE}
pipedat %>% ggplot(aes(x=year, y=emissions, color=Party)) + 
  geom_point() 
```
...
fakeGDP <- data.frame(unique(pipedat$Party), sample(1e4:1e5, length(unique(pipedat$Party)))) %>%
  rename(Party = V1, fakeGDP = V2) ## GET THIS TO HAVE BETTER COLUMN NAMES 
